...
from models import User,Admin,Book,Issue
...
def issue():
  ...
  if form.validate_on_submit():
    ...
    if bid and sid and (bid.copies>=int(form.copies.data)):
      ...
      db.session.commit()
      is1=Issue(issuedcopies=int(form.copies.data),type='Issued',book_id=bid.id)
      db.session.add(is1)
      db.session.commit()
...
def returnf():
  ...
  if form.validate_on_submit():
    ...
    if bid and sid and (currentissuedcopies>=int(form.copies.data)):
      ...
      bid.issuedcopies-=int(form.copies.data)
      db.session.commit()
      is1=Issue(issuedcopies=int(form.copies.data),type='Return',book_id=bid.id)
      db.session.add(is1)
      db.session.commit()
